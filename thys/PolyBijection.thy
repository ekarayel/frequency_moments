...
proof (induction n arbitrary: f)
  ...
  proof (cases "xs=[]")
    case True
    hence "K = {}" using xs_def by simp
    hence "[] \<in> {\<omega> \<in> bounded_degree_polynomials F (card K+0). \<forall>x \<in> K. ring.eval F \<omega> x = f x}" 
      by (simp add:bounded_degree_polynomials_def univ_poly_zero_closed)
    moreover have "finite  {\<omega> \<in> bounded_degree_polynomials F (card K + 0). \<forall>x \<in> K. ring.eval F \<omega> x = f x}"
      by (simp add: fin_degree_bounded assms(1) assms(2))
    ultimately have "1 \<le> card {\<omega> \<in> bounded_degree_polynomials F (card K+0). \<forall>x \<in> K. ring.eval F \<omega> x = f x}"
  ...
  next
    case False
    hence "1 \<le> length xs" 
      by (metis One_nat_def Suc_le_mono add_Suc_right list.exhaust list.size(4) zero_le)
    moreover have "set xs \<subseteq> carrier F" using xs_def assms(3) by auto
    ...
    ultimately have "interpolate F f xs \<in> {\<omega> \<in> bounded_degree_polynomials F (length xs). \<forall>x \<in> K. ring.eval F \<omega> x = f x}" 
      apply (simp add:bounded_degree_polynomials_def del:interpolate.simps)
      using interpolate_l assms(1) 0 xs_distinct 
      by (metis One_nat_def xs_def)
...
next
  ...
  proof -
    ...
    moreover have "finite (group l)" 
      by (simp add: fin_degree_bounded assms(2) local.field_axioms group_def)
  ...
  moreover have "sum (\<lambda>l. card (group l)) (carrier F) = card (\<Union>l \<in> carrier F. group l)"
    ...
    apply (simp add:fin_degree_bounded group_def assms)
