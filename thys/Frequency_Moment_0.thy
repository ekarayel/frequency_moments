...
proof -
  ...
  have "Max (sketch_rv' \<omega>) = real_of_float (Max (f0_sketch \<omega>))" if a:"card (sketch_rv' \<omega>) \<ge> t" for \<omega> 
  proof -
  ...
  qed
  hence real_g: "\<And>\<omega>. estimate' (sketch_rv' \<omega>) = real_of_rat (estimate (f0_sketch \<omega>))"
