...
qed
*)
text \<open>Bijection between @{term "{..<n}"} and @{term "ZFact (int n)"}.\<close>
...
lemma zfact_prime_is_field:
  ...
  shows "field (ZFact (int p))"
proof -
  have "finite (carrier (ZFact (int p)))" using zfact_finite assms prime_gt_0_nat by blast
  moreover have "domain (ZFact (int p))" using ZFact_prime_is_domain assms by auto
  ultimately show ?thesis using finite_domains_are_fields by blast
qed
...
proof (rule ring_iso_memI)
  ...
  show "\<And>x. x \<in> carrier (ZFact (int n)) \<Longrightarrow> zfact_iso_inv n x \<in> carrier (mod_ring n)"
  proof -
    fix x 
    assume "x \<in> carrier (ZFact (int n))"
    hence "zfact_iso_inv n x \<in> {..<n}"
      unfolding zfact_iso_inv_def using zfact_iso_ran[OF n_ge_0] inv_into_into by metis
  ...
  show "\<And>x y. x \<in> carrier (ZFact (int n)) \<Longrightarrow> y \<in> carrier (ZFact (int n)) \<Longrightarrow>
    zfact_iso_inv n (x \<otimes>\<^bsub>ZFact (int n)\<^esub> y) = zfact_iso_inv n x \<otimes>\<^bsub>mod_ring n\<^esub> zfact_iso_inv n y"
  proof -
    fix x y
    ...
    have "x \<otimes>\<^bsub>ZFact (int n)\<^esub> y = (I +>\<^bsub>\<Z>\<^esub> (int x')) \<otimes>\<^bsub>ZFact (int n)\<^esub> (I +>\<^bsub>\<Z>\<^esub> (int y'))"
      unfolding x'_def y'_def using x_carr y_carr zfact_coset[OF n_ge_0] I_def by simp
    ...
    thus "zfact_iso_inv n (x \<otimes>\<^bsub>ZFact (int n)\<^esub> y) = zfact_iso_inv n x \<otimes>\<^bsub>mod_ring n\<^esub> zfact_iso_inv n y"
      using a x'_def y'_def by simp
  qed

  show "\<And>x y. x \<in> carrier (ZFact (int n)) \<Longrightarrow> y \<in> carrier (ZFact (int n)) \<Longrightarrow>
    zfact_iso_inv n (x \<oplus>\<^bsub>ZFact (int n)\<^esub> y) = zfact_iso_inv n x \<oplus>\<^bsub>mod_ring n\<^esub> zfact_iso_inv n y"
  proof -
    fix x y
    assume x_carr: "x \<in> carrier (ZFact (int n))"
    define x' where "x' = zfact_iso_inv n x"
    ...
    have "zfact_iso_inv n (zfact_iso n (x' \<oplus>\<^bsub>mod_ring n\<^esub> y')) = x' \<oplus>\<^bsub>mod_ring n\<^esub> y'" 
      unfolding zfact_iso_inv_def
      by (rule inv_into_f_f[OF zfact_iso_inj[OF n_ge_0] b])
    thus "zfact_iso_inv n (x \<oplus>\<^bsub>ZFact (int n)\<^esub> y) = zfact_iso_inv n x \<oplus>\<^bsub>mod_ring n\<^esub> zfact_iso_inv n y"
      using a x'_def y'_def by simp
  qed
  ...
  show "bij_betw (zfact_iso_inv n) (carrier (ZFact (int n))) (carrier (mod_ring n))"
    using zfact_iso_inv_def mod_ring_def zfact_iso_bij[OF n_ge_0] bij_betw_inv_into by force
qed
...
proof -
  ...
  moreover have "(mod_ring n) \<lparr> zero := zfact_iso_inv n \<zero>\<^bsub>ZFact (int n)\<^esub> \<rparr> = mod_ring n"
...
proof -
  ...
  have "inv_into (carrier (ZFact (int n))) (zfact_iso_inv n) \<in> ring_iso (mod_ring n) (ZFact (int n))"
...
lemma mod_ring_is_field:
  ...
  shows "field (mod_ring n)"
proof -
  have n_ge_0: "n > 0" using assms prime_gt_0_nat by blast
  have n_ge_1: "n > 1" using assms prime_gt_1_nat by blast

  interpret field "ZFact (int n)"
    using zfact_prime_is_field[OF assms] by simp

  have "field ((mod_ring n) \<lparr> zero := zfact_iso_inv n \<zero>\<^bsub>ZFact (int n)\<^esub> \<rparr>)"
    by (rule ring_iso_imp_img_field[OF zfact_iso_inv_is_ring_iso[OF n_ge_1]])
