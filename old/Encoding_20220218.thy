...
text \<open>Encoding for natural numbers within a given range\<close>

fun encode_bounded_nat_aux :: "nat \<Rightarrow> nat \<Rightarrow> bool list"
  where
    "encode_bounded_nat_aux 0 _ = []" |
    "encode_bounded_nat_aux (Suc n) x = odd x#(encode_bounded_nat_aux n (x div 2))"

lemma "inj_on (encode_bounded_nat_aux n) {..<2^n}"
  apply (induction n, simp add:inj_on_def, simp add:inj_on_def) sorry
...
proof -
  have "bit_count (set\<^sub>S f S) \<le> ereal (length (sorted_list_of_set S)) * (a+1) + 1"
    using assms(1,4) list_bit_count_est[where xs="sorted_list_of_set S"] by simp
